...
public class ControllerAndHandSetup : MonoBehaviour
{
        ...
        for (int i = 0; i <= (int)WebXRHandJoint.pinky_finger_tip; i++)
        {
          ...
          handJoints[i].SetLocalPositionAndRotation(rotationOffset * (handData.joints[i].position - handData.joints[0].position), rotationOffset * handData.joints[i].rotation);
            ...
            else
            {
                ...
                //turn the tips RED and add a collider
                if(i == (int)WebXRHandJoint.index_finger_tip || i == (int)WebXRHandJoint.middle_finger_tip || i == (int)WebXRHandJoint.ring_finger_tip
                    || i == (int)WebXRHandJoint.ring_finger_tip || i == (int)WebXRHandJoint.thumb_tip)
                {
